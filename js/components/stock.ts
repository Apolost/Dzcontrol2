...
export function handleSaveBoxType() {
    ...
    if (!name) {
        showToast('Název bedny nesmí být prázdný.', 'error');
        return;
    }
    
    const isDuplicate = appState.boxTypes.some(bt => bt.name.toLowerCase() === name.toLowerCase() && bt.id !== id);
    if (isDuplicate) {
        showToast('Bedna s tímto názvem již existuje.', 'error');
        return;
    }
...
export function handleDeleteBoxType(id) {
    ...
    if (isInUse) {
        showToast('Tento typ bedny nelze smazat, protože je přiřazen k produktům.', 'error');
        return;
    }
...
export function handleSaveBoxAssignments() {
    ...
    saveState();
    showToast('Přiřazení beden uloženo.');
...
export function handleSaveTrayType() {
    ...
    if (!name) {
        showToast('Název misky nesmí být prázdný.', 'error');
        return;
    }
    
    if (appState.trayTypes.some(tt => tt.name.toLowerCase() === name.toLowerCase() && tt.id !== id)) {
        showToast('Miska s tímto názvem již existuje.', 'error');
        return;
    }
    ...
    if (id) {
        ...
        showToast('Typ misky upraven.');
    } else {
        appState.trayTypes.push({ id: `mt${Date.now()}`, name });
        showToast('Nový typ misky přidán.');
    }
...
export function handleDeleteTrayType(id) {
    if (Object.values(appState.customerTrayAssignments).some(cust => Object.values(cust).some(trayId => trayId === id))) {
        showToast('Tento typ misky nelze smazat, protože je přiřazen k produktům.', 'error');
        return;
    }
...
export function handleSaveTrayAssignments() {
    ...
    saveState();
    showToast('Přiřazení misek uloženo.');
    renderStockTrays();
}
